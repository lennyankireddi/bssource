...
function RemoveSearchEnterEvent()
{
	...
	jQuery(document).on("keyup", ".search-everything input", function(e) {
		if (e.which == 13 && jQuery(this).val().length > 0) {
			window.location = "/search/Pages/results.aspx?k=filename:" + jQuery(this).val();
		}
	...
	jQuery(document).on("click", ".ms-srch-sb-searchLink", function(e){
		window.location = "/search/Pages/results.aspx?k=filename:" + jQuery(".search-everything input").val();
	});
...
function LanguageHandler()
{
	...
	switch(_spPageContextInfo.currentCultureName)
	{
		...
		break;
		case "fr-CA":
			jQuery('#site_follow_button .ms-promotedActionButton-text').text("Pr�f�r�");
...
function RichDocumentSearch(e) {
    ...
    t = t.replace(" ", "*");
    t = t + "*";
	//var searchUrl = "/_api/search/query?querytext='(" + t + " Path:" + url + "/Beam*)+OR+(" + t + " contentclass:STS_Web Path:" + url + "/" + subsite + ")'&selectproperties='Path,ServerRedirectedURL,Title,FileExtension'"
	//var searchUrl = "/_api/search/query?querytext='" + t + " (IsDocument:True OR contentclass:STS_Web) Path:" + url + "/" + subsite + "'&selectproperties='Path,ServerRedirectedURL,Title,FileExtension'";
	...
	console.log('site absolute url: ' + _spPageContextInfo.siteAbsoluteUrl);
    ...
    jQuery.ajax({
        ...
        success: function(e) {
			...
			t += '<div class="srch-view-more"><a href="/search/pages/results.aspx?k=filename:' + searchTerm + '">View more...</a></div>';
