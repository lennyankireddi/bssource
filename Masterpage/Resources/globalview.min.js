...
function GetTranslation(input, lang, callback) {
	lookupUrl = _spPageContextInfo.siteAbsoluteUrl + "/_api/web/lists/getbytitle('Translations')/items?$filter=Title eq '" + input + "'";
	var text = "";
	jQuery.ajax({
		url: lookupUrl,
		method: "GET",
		...
		success: function(data) {
			if (data.d.results.length > 0) {
				...
				switch(lang.toLowerCase()) {
					case "connect": text = item.Title;
						break;
					case "de": text = item.LANG_DE;
						break;
					case "es": text = item.LANG_ES;
						break;
					case "fr": text = item.LANG_FR;
						break;
					case "ja": text = item.LANG_JA;
						break;
					case "pl": text = item.LANG_PL;
						break;
					case "pt": text = item.LANG_PT;
						break;
					case "ru": text = item.LANG_RU;
						break;
					case "zhcn": text = item.LANG_ZHCN;
						break;
					case "zhhk": text = item.LANG_ZHHK;
						break;
...
{
	//hide navigation items based on SuccessFactorsRoleID attribute
	if (claimsJSONMap.SuccessFactorsRoleID !== undefined)
	{
		...
		{
			//hide our brands for trade users
			var rackhouse = jQuery('#DeltaTopNavigation ul.root > li.static:nth-child(1)').css('display','none');
		}
	}
